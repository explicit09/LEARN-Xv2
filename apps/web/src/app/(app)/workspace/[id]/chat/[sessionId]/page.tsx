...
export default async function ChatSessionPage({ params }: Props) {
  ...
  // Load real workspace documents for the sources sidebar
  const { data: documents } = await supabase
    .from('documents')
    .select('id, title, file_type, token_count')
    .eq('workspace_id', workspaceId)
    .eq('status', 'ready')
      ...
      <header className="h-16 border-b border-border/50 bg-background/80 backdrop-blur-xl shrink-0 flex items-center justify-between px-6 z-20 sticky top-0">
        <div className="flex items-center gap-4">
          <Link
            ...
            className="text-muted-foreground hover:text-foreground transition-colors p-2 -ml-2 rounded-lg hover:bg-muted/50"
          >
        ...
        <div className="flex items-center gap-3">
          <Button
            variant="outline"
            className="hidden sm:inline-flex h-9 rounded-lg border-border bg-transparent hover:bg-muted/50 font-semibold text-xs"
          >
          ...
          <Button
            variant="destructive"
            className="h-9 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500/20 border border-red-500/20 font-semibold text-xs shadow-none"
          >
            End Session
              ...
              {(documents ?? []).length === 0 ? (
                ...
                (documents ?? []).map((doc, i) => (
                  <div
                    ...
                    className="group rounded-xl p-3 bg-muted/30 border border-border/50 hover:bg-muted/50 hover:border-border transition-colors cursor-pointer relative overflow-hidden"
                  >
                    <div
                      className={`absolute left-0 top-0 bottom-0 w-1 ${i === 0 ? 'bg-emerald-500' : 'bg-yellow-500'}`}
                    />
                    <div className="flex justify-between items-start mb-2 pl-2">
                      <span
                        className={`text-[10px] font-bold tracking-wider uppercase flex items-center gap-1 ${i === 0 ? 'text-emerald-500' : 'text-yellow-500'}`}
                      >
                    ...
                    <div className="pl-2 flex items-center gap-2">
                      ...
                      <span className="text-sm font-semibold truncate">{doc.title as string}</span>
                    </div>
                  </div>
                ))
              )}
