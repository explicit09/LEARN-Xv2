    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div
            key={i}
            className="h-40 animate-pulse rounded-2xl bg-muted/50 border border-border"
          />
        ))}
      </div>
    )
    ...
    return (
      <div className="flex flex-col items-center justify-center p-16 text-center rounded-3xl border border-dashed border-border/50 bg-card/20 backdrop-blur-sm m-4">
        ...
        <p className="text-muted-foreground max-w-sm">
          The AI engine will automatically build a knowledge graph of core concepts once your
      ...
      {concepts.map((concept) => (
        ...
        >
          <div>
            ...
            <h3 className="text-lg font-black text-foreground mb-2 line-clamp-1">
