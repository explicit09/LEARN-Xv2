...
function StatCard({
  ...
  return (
    <div className="glass-card overflow-hidden rounded-2xl border border-border/50 p-3 sm:p-5 relative group">
      <div
        className={`absolute top-0 right-0 w-24 h-24 rounded-full blur-2xl -mr-10 -mt-10 opacity-20 group-hover:opacity-40 transition-opacity ${colorClass.replace('text-', 'bg-')}`}
      />
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4">
          <div
            className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center border shadow-inner ${colorClass.replace('text-', 'bg-').replace('500', '500/10')} ${colorClass} ${colorClass.replace('text-', 'border-').replace('500', '500/20')}`}
          >
            <Icon className="w-5 h-5" />
          </div>
        </div>
        <p className="text-2xl sm:text-3xl font-black text-foreground mb-1">{value}</p>
...
export function MasteryDashboard({ workspaceId }: MasteryDashboardProps) {
      ...
      <div className="space-y-8 p-4">
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-32 animate-pulse rounded-2xl bg-muted/50 border border-border"
            />
          ))}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6 sm:space-y-10 p-3 sm:p-4">
      {/* Summary stats */}
      <div className="grid min-w-0 grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="Total Concepts"
          value={summary?.totalConcepts ?? 0}
          icon={BookOpen}
        ...
        <StatCard
          label="Due Reviews"
          value={summary?.dueReviews ?? 0}
          sub="Flashcards waiting"
          icon={CalendarClock}
              ...
              {next.map((item) => {
                ...
                return (
                  <Link
                    key={item.id}
                    href={href}
                    className="flex items-center justify-between rounded-2xl border border-border/50 bg-card/60 backdrop-blur-sm p-3 sm:p-5 hover:border-primary/30 transition-all group shadow-sm"
                  >
                    <div className="flex items-start gap-3 sm:gap-4 min-w-0">
                      <div
                        className={`mt-1 w-8 h-8 rounded-full flex items-center justify-center border shrink-0 ${isFlashcard ? 'bg-amber-500/10 text-amber-500 border-amber-500/20' : 'bg-primary/10 text-primary border-primary/20'}`}
                      >
                        {isFlashcard ? (
            ...
            <div className="space-y-3">
              {weak.map((c) => {
                ...
                return (
                  <div
                    key={c.concept_id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 rounded-2xl border border-border/50 bg-card/60 backdrop-blur-sm p-3 sm:p-5 shadow-sm"
                  >
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">
                          {Math.round(c.avg_lapses)} avg lapses
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <span className="w-10 text-right text-xs font-bold font-mono">
                        {stability}%
                      </span>
                    </div>
                  </div>
