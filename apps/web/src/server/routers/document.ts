  ...
  confirmUpload: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const { data: doc, error: fetchError } = await ctx.supabase
        ...
        .eq('user_id', userId)
        .single()
      if (fetchError || !doc) throw new TRPCError({ code: 'NOT_FOUND', message: 'Document not found' })
  ...
  get: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const { data: document, error } = await ctx.supabase
        ...
        .eq('user_id', userId)
        .single()
      if (error || !document) throw new TRPCError({ code: 'NOT_FOUND', message: 'Document not found' })
