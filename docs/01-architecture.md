...
- unpdf + Mammoth + Gemini OCR fallback for document parsing
- Claude Sonnet 4.6 via Vercel AI SDK for lessons, chat, concept extraction
- Gemini 3.1 Flash-Lite via Vercel AI SDK for quizzes, flashcards, study guides
...
- Need Docling for table-heavy documents (better table extraction than unpdf)
...
| ----------- | ------------------ | ----------------------- | ---------------------- | ----------------------------- |
| Local dev   | Next.js dev server | Supabase local (Docker) | Trigger.dev dev CLI    | —                             |
