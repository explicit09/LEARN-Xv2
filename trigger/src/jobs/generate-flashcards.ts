...
import { getProvider, MODEL_ROUTES, calculateCost } from '../lib/ai'
import {
...
export const generateFlashcards = task({
  ...
  run: async (payload: GenerateFlashcardsPayload) => {
    try {
      ...
      const MODEL = MODEL_ROUTES.FLASHCARD_GENERATION
      ...
      await supabase.from('ai_requests').insert({
        ...
        cost_usd: calculateCost(MODEL, usage.inputTokens ?? 0, usage.outputTokens ?? 0),
      ...
      logger.error('generate-flashcards CRASHED', {
        error: String(err),
